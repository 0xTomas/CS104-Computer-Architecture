...
class UltraSuperCalculator:
    ...
    def store_value_to_register(self, value_to_store):
        if self.numbers_index > 21:
            self.numbers_index = 1
        integer = int(value_to_store, 2)
        self.number_registers[self.numbers_index] = integer
        print(f"{integer} has been storned in Register: {self.numbers_index}.")
        self.numbers_index += 1
    ...
    def get_last_calculation(self):
        ...
        value = self.history_registers[self.temp_history_index]
        last_value = f"The last result was: {value}"
    ...
    def binary_reader(self, instruction):
        ...
        self.update_display(f"The result of the {calculation} is {result}!")


calc = UltraSuperCalculator("Tom")

# Adds 5 and 10 to the number registers:
